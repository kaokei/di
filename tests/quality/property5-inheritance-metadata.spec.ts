 ...
 * 通过 getMetadata(KEYS.INJECTED_PROPS, ChildCtor) 获取的元数据应包含父类和子类的
 ...
 * - 验证子类同名属性覆盖父类声明
 * - 验证三层继承链的逐层合并
 * - 验证 getOwnMetadata 只获取当前类自己的元数据
 *
...
test('Property 5: 父类有属性 A，子类有属性 B，getMetadata(子类) 应包含 A 和 B', () => {
  fc.assert(
    fc.property(
...
test('Property 5: 父类有属性 A（Token1），子类也有属性 A（Token2），getMetadata(子类) 中 A 应为 Token2', () => {
  fc.assert(
    fc.property(
      arbPropName,
      arbTokenName,
      ...
      (propName, tokenName1, tokenName2) => {
        ...
        // getMetadata(子类) 中该属性应为子类的 token2
        const childMeta = getMetadata(KEYS.INJECTED_PROPS, Child);
        expect(childMeta).toBeDefined();
...
test('Property 5: 三层继承链（祖父类 → 父类 → 子类），getMetadata 应逐层合并所有属性', () => {
  fc.assert(
    fc.property(
      ...
      ([propA, propB, propC], tokenNameA, tokenNameB, tokenNameC) => {
        ...
        // getMetadata(祖父类) 应只包含自己的属性
        const grandMeta = getMetadata(KEYS.INJECTED_PROPS, GrandParent);
...
// ==================== getOwnMetadata 不包含继承的元数据 ====================

test('Property 5: getOwnMetadata 只获取当前类自己的元数据，不包含父类的声明', () => {
  fc.assert(
    fc.property(
      arbTwoDistinctPropNames,
      arbTokenName,
      ...
      ([propA, propB], tokenNameA, tokenNameB) => {
        ...
        // getOwnMetadata(子类) 应只包含子类自己的属性 B
        const childOwnMeta = getOwnMetadata(KEYS.INJECTED_PROPS, Child);
        expect(childOwnMeta).toBeDefined();
        expect(childOwnMeta![propB]).toBeDefined();
        expect(childOwnMeta![propB][KEYS.INJECT]).toBe(tokenB);
        expect(childOwnMeta![propA]).toBeUndefined();

        // getMetadata(子类) 应包含父类和子类的属性
        const childMeta = getMetadata(KEYS.INJECTED_PROPS, Child);
        expect(childMeta).toBeDefined();
        expect(childMeta![propA]).toBeDefined();
        expect(childMeta![propB]).toBeDefined();
  ...
  fc.assert(
    fc.property(
      ...
      (propName, tokenName1, tokenName2) => {
        ...
        // 子类的 getMetadata 中，该属性应使用子类的声明（不带 optional）
        const childMeta = getMetadata(KEYS.INJECTED_PROPS, Child);
        expect(childMeta![propName][KEYS.INJECT]).toBe(token2);
