 ...
 * decorate() 动态应用于该方法后，通过 getMetadata(KEYS.POST_CONSTRUCT, Ctor) 获取的元数据应为
 * { key: methodName, value: param }；将 @PreDestroy() 应用于方法后，通过
 * getMetadata(KEYS.PRE_DESTROY, Ctor) 获取的元数据应为 { key: methodName, value: undefined }。
 *
 * 测试策略：
 * - 使用 decorate() 辅助函数动态应用 @PostConstruct / @PreDestroy 装饰器
 * - 使用 fast-check 生成随机方法名和 PostConstruct 参数
 * - 验证 getMetadata(KEYS.POST_CONSTRUCT, Ctor) 返回 { key: methodName, value: param } 格式
 * - 验证 getMetadata(KEYS.PRE_DESTROY, Ctor) 返回 { key: methodName, value: undefined } 格式
...
test('Property 9.1: 使用 decorate() 应用 @PostConstruct，对于任意方法名和参数，元数据应以 { key, value } 格式存储', () => {
  fc.assert(
    fc.property(arbMethodName, arbPostConstructParam, (methodName, param) => {
      ...
      // 验证元数据格式
      const metadata = getMetadata(KEYS.POST_CONSTRUCT, TestService) as { key: string; value?: PostConstructParam } | undefined;
...
test('Property 9.2: 使用 decorate() 应用 @PreDestroy，对于任意方法名，元数据应以 { key, value: undefined } 格式存储', () => {
  fc.assert(
    fc.property(arbMethodName, (methodName) => {
      ...
      const metadata = getMetadata(KEYS.PRE_DESTROY, TestService) as { key: string } | undefined;
      expect(metadata).toBeDefined();
  ...
  fc.assert(
    fc.property(
      ...
      (initMethod, destroyMethod, param) => {
        ...
        const pcMeta = getMetadata(KEYS.POST_CONSTRUCT, TestService) as { key: string; value?: PostConstructParam } | undefined;
        expect(pcMeta).toBeDefined();
        expect(pcMeta).toEqual({ key: initMethod, value: param });

        // 验证 @PreDestroy 元数据
        const pdMeta = getMetadata(KEYS.PRE_DESTROY, TestService) as { key: string } | undefined;
