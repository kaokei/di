...
describe('Feature: code-quality-optimization, Property 4: 元数据深拷贝隔离', () => {
  test('对于任意父子类，修改子类的 INJECTED_PROPS 元数据不影响父类', () => {
    fc.assert(
      fc.property(
        ...
        (parentProp, childProp, extraKey) => {
          ...
          if (childMeta && childMeta[parentProp]) {
            childMeta[parentProp][extraKey] = 'mutated';
          }

          // 验证父类元数据未被修改（使用 Object.hasOwn 避免原型链属性干扰）
          const parentMeta = getOwnMetadata(KEYS.INJECTED_PROPS, Parent) as Record<string, Record<string, unknown>>;
          if (parentMeta && parentMeta[parentProp]) {
            expect(Object.hasOwn(parentMeta[parentProp], extraKey)).toBe(false);
          }
        }
      ),
      { numRuns: 100 }
