  ...
  fc.assert(
    fc.property(arbTokenName, (tokenName) => {
      ...
      try {
        ...
        const metaA = getMetadata(KEYS.INJECTED_PROPS, ClassA);
        const metaB = getMetadata(KEYS.INJECTED_PROPS, ClassB);
...
test('Property 4: 对于任意参数，decorate(PostConstruct(param)) 与 @PostConstruct(param) 语法应产生相同的 POST_CONSTRUCT 元数据', () => {
  fc.assert(
    fc.property(arbPostConstructParam, (param) => {
      ...
      const metaA = getOwnMetadata(KEYS.POST_CONSTRUCT, ClassA);
      const metaB = getOwnMetadata(KEYS.POST_CONSTRUCT, ClassB);
...
test('Property 4: decorate(PreDestroy()) 与 @PreDestroy() 语法应产生相同的 PRE_DESTROY 元数据', () => {
  fc.assert(
    fc.property(fc.boolean(), (_flag) => {
      ...
      const metaA = getOwnMetadata(KEYS.PRE_DESTROY, ClassA);
      const metaB = getOwnMetadata(KEYS.PRE_DESTROY, ClassB);
