...
3. 至于分成两步`存入cache`和`更新cache`。

这里必须要求 activation 只能 mutable 修改 cache 中的对象，而不能替换成新的对象。
...
这个流程可以说是最符合语义，或者说最符合大多数人理解的流程。但是这么做导致的最大问题就是完全不支持属性注入的循环依赖了。
