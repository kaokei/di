...
2. 另一种场景则是上面的示例 5。是在运行时发生的循环依赖。一般是在类的实例化过程中发生的。也就是在执行`const a = container.get(A)`这行代码时，可能会发生循环依赖。[查看 resolveInstanceValue 源代码](https://github.com/kaokei/di/blob/245f549d4bcd85ce619a7fdc4a99f221dcdb57ef/src/binding.ts#L244)

#### inversify 执行 resolveInstanceValue 的过程

```
获取构造函数参数依赖数组（inversify 支持构造函数参数注入）
new ClassName(...args)
...
1. 构造函数的参数导致的循环依赖（inversify 支持构造函数参数注入）
2. 属性注入导致的循环依赖
...
> 注意：本库当前版本基于 Stage 3 装饰器规范，不支持构造函数参数注入（Stage 3 规范不支持参数装饰器），因此不存在构造函数参数导致的循环依赖问题。

```
new ClassName()（无参构造，不再支持构造函数参数注入）
...
注册实例到容器映射（_registerInstance）

获取所有属性注入依赖（_getInjectProperties）
执行PostConstruct逻辑（_postConstruct）
```

观察到`存入cache`这一步是在倒数第3步，所以前面所有步骤都可能导致循环依赖。包括以下这些：

1. binding activation 逻辑导致的循环依赖
2. container activation 逻辑导致的循环依赖
